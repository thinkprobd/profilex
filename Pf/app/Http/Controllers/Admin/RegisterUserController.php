...
class RegisterUserController extends Controller
{
    ...
    {
        $languages = ModelsLanguage::get();
        $language = ModelsLanguage::where('code', $request->language)->first();
        $data = BasicSetting::where('language_id', $language->id)->select('id', 'user_login_attempts', 'user_registraion_status', 'user_registration_deactive_img', 'user_registration_deactive_text', 'user_registraion_countries')
            ->first();
        return view('admin.register_user.settings', compact('data', 'languages', 'language'));
    }
    public function settingsUpdate(Request $request, $id)
    {

        $bs = BasicSetting::find($id);
        // dd($bs);
        $request->validate([
            'user_registraion_status' => 'required|integer',
            'user_login_attempts' => 'required|integer',
            'image' => [Rule::requiredIf(function () use ($bs) {

                return $bs->user_registration_deactive_img == null;
            }), 'file', 'mimes:jpg,png,jpeg,svg,gif'],

            'user_registration_deactive_text' => 'required'
        ], [
            'language_id.required' => "The language field is required."
        ]);

        $filename = $bs->user_registration_deactive_img;
        if ($request->hasFile('image')) {
            @unlink(public_path('assets/front/img/' . $bs->user_registration_deactive_img));
        ...
        $bs->update([
            'user_registraion_status' => $request->user_registraion_status,
            'user_registration_deactive_img' => $filename,
            'user_login_attempts' => $request->user_login_attempts,
            'user_registration_deactive_text' => $request->user_registration_deactive_text,
            'user_registraion_countries' => $request->user_registraion_countries
        ]);

        Session::flash('success', __('User Management') . ' ' . __('Settings updated successfully!'));
    ...
    public function delete(Request $request)
    {
        ...
        if ($user->vcards()->count() > 0) {
            $vcards = $user->vcards;
            $deleteVcard = new VcardController();
            foreach ($vcards as $key => $vcard) {
                $deleteVcard->deleteVcard($vcard->id);
    ...
    public function changeCurrPackage(Request $request)
    {
        ...
        if (!empty($nextMembership) && $selectedPackage->term == 'lifetime') {
            Session::flash('membership_warning', __('To add a Lifetime package as Current Package') . ', ' . __('You have to remove the next package'));
            return back();
        }
    ...
    public function addNextPackage(Request $request)
    {
        ...
        $allowedExts = array('change', 'remove', 'approve', 'reject');
        if ($hasPendingMemb) {
            Session::flash('membership_warning', __('This user already has a Pending Package') . '. ' . __('Please take an action') . ' ' . implode('/ ', __($allowedExts)) . ' ' . __('for that package first'));
            return back();
        }
        ...
        $bs = BasicSetting::select('website_title')->first();
        $selectedPackage = Package::find($request->package_id);
        $allowedChangeData = array('change', 'remove');

        if ($currMembership->is_trial == 1) {
            Session::flash('membership_warning', __('If your current package is trial package') . ', ' . __('then you have to') . ' ' . implode('/ ', $allowedChangeData) . ' ' . __('the current package first'));
            return back();
        }
